...
/* ─── PLAN TAB CONTENT ──────────────────────────────────── */
function PlanContent({ plan, isDark = false, onOpenComparison }: { plan: PlanData; isDark?: boolean; onOpenComparison: () => void }) {
        ...
        <div className="max-w-7xl mx-auto space-y-14">
          ...
          <div className={`${isDark ? "bg-white/5 border-white/10" : "bg-brand-dark border-white/10"} p-10 md:p-12 rounded-[3rem] shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-8 border relative overflow-hidden group`}>
            ...
            <div className="flex flex-wrap gap-4 relative z-10 shrink-0">
              ...
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2a4 4 0 00-4-4H5m11 4v2a4 4 0 004 4h1m-1-4l-3 3m3-3l3 3M9 21v-5a2 2 0 012-2h2a2 2 0 012 2v5m-6 0h6" /></svg>
                COMPARATIVO
              </button>
  ...
  const tabsContent = [
    {
      title: "Básico",
      value: "basico",
      content: <PlanContent plan={plans[0]} onOpenComparison={() => setIsComparisonOpen(true)} />,
    },
    {
      title: "Estándar",
      value: "estandar",
      content: <PlanContent plan={plans[1]} onOpenComparison={() => setIsComparisonOpen(true)} />,
    },
    {
      title: "Premium",
      value: "premium",
      content: <PlanContent plan={plans[2]} onOpenComparison={() => setIsComparisonOpen(true)} />,
    },
    {
      title: "Enterprise",
      value: "enterprise",
      content: <PlanContent plan={plans[3]} isDark onOpenComparison={() => setIsComparisonOpen(true)} />,
    },
  ];
      ...
      <AnimatePresence>
        {isComparisonOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-brand-dark/95 backdrop-blur-md flex items-center justify-center p-4 md:p-10"
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-3xl w-full max-w-6xl max-h-[90vh] overflow-hidden shadow-2xl relative flex flex-col pt-16"
              onClick={(e) => e.stopPropagation()}
            >
              <button 
                onClick={() => setIsComparisonOpen(false)}
                className="absolute top-6 right-6 text-neutral-400 hover:text-brand-dark transition-colors p-2"
              >
              ...
              <div className="flex-1 overflow-auto p-8 md:p-12">
                <div className="text-center mb-12">
                  <h3 className="text-3xl font-black text-brand-dark tracking-tighter mb-2">Tabla Comparativa de Planes</h3>
                  <p className="text-neutral-500 font-medium">Compara detalladamente los beneficios y costos de cada nivel de suscripción.</p>
                </div>

                <div className="w-full overflow-x-auto rounded-2xl border border-neutral-100">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-neutral-50">
                        <th className="p-6 text-sm font-black text-brand-dark uppercase tracking-wider border-b border-neutral-100">Características</th>
                        {plans.map((plan, i) => (
                          <th key={i} className="p-6 text-center border-b border-neutral-100">
                            <div className={`text-xs font-black uppercase tracking-widest mb-1 ${plan.accentColor}`}>Plan {plan.nivel}</div>
                            <div className="text-xl font-black text-brand-dark">{plan.name}</div>
                          </th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-100">
                      <tr>
                        <td className="p-6 text-sm font-bold text-neutral-600 bg-neutral-50/30">Costo Mensual</td>
                        {plans.map((plan, i) => (
                          <td key={i} className="p-6 text-center font-black text-lg text-brand-dark">${plan.price}</td>
                        ))}
                      </tr>
                      <tr>
                        <td className="p-6 text-sm font-bold text-neutral-600 bg-neutral-50/30">Costo Anual (Avg)</td>
                        {plans.map((plan, i) => (
                          <td key={i} className="p-6 text-center font-bold text-neutral-500">${plan.annualPrice}/mes</td>
                        ))}
                      </tr>
                      <tr>
                        <td className="p-6 text-sm font-bold text-neutral-600 bg-neutral-50/30">Frecuencia Reportes</td>
                        {plans.map((plan, j) => (
                          <td key={j} className="p-6 text-center text-sm font-medium">
                            {plan.kpis.find(k => k.label === "Frecuencia de Reporte")?.value || "N/A"}
                          </td>
                        ))}
                      </tr>
                      <tr>
                        <td className="p-6 text-sm font-bold text-neutral-600 bg-neutral-50/30">Alcance / Soporte</td>
                        {plans.map((plan, j) => (
                          <td key={j} className="p-6 text-center text-sm font-medium">
                            {plan.kpis.find(k => k.label === "Alcance" || k.label === "Soporte" || k.label === "Sectores")?.value || "Estándar"}
                          </td>
                        ))}
                      </tr>
                      <tr>
                        <td className="p-6 text-sm font-bold text-neutral-600 bg-neutral-50/30">Servicios Clave</td>
                        {plans.map((plan, i) => (
                          <td key={i} className="p-6 align-top">
                            <ul className="space-y-2">
                              {plan.features.slice(0, 4).map((f, k) => (
                                <li key={k} className="flex items-start gap-2 text-[11px] leading-tight text-neutral-600">
                                  <span className={plan.accentColor}>•</span>
                                  {f.label}
                                </li>
                              ))}
                            </ul>
                ...
                <div className="mt-12 p-8 bg-brand-primary/5 rounded-3xl border border-brand-primary/10 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div>
                    <h4 className="font-black text-brand-dark text-lg mb-1">¿Necesitas una asesoría personalizada?</h4>
                    <p className="text-neutral-500 text-sm font-medium">Nuestro equipo técnico puede ayudarte a elegir el plan que mejor se adapte a tu operación.</p>
                  </div>
                  <button className="px-10 py-4 bg-brand-primary text-white rounded-2xl font-black shadow-lg hover:scale-105 transition-all whitespace-nowrap">
                    CONTACTAR VENTAS
                  </button>
                </div>
              </div>
